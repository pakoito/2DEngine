						  ...
						  blendAdditive:(BOOL)inBlendAdditive
{
	...
	if (self != nil) {
		...
		if(inTextureName) {		
			...
			texture = [[Image alloc] initWithTexture:inTextureName  filter:GL_NEAREST Use32bits:NO TotalVertex:(maxParticles*12)];
		}
...
- (void)update:(GLfloat)delta {
	...
	//If we put 1.0 in the rate, particles are being generated one by one
	//I want all of them at the same time, so I put 0.1f
	//these maybe needs to be changed depends on the effect we want to make
...
- (void)renderParticles {
	...
	if(texture && [_sharedTexture currentlyBoundTexture] != [[texture texture] name]) {
...
- (void)renderParticlesWithoutTexture {
...
/*
 ...
 EXAMPLE EXPLOSION
particles = [[ParticleEmitter alloc] initParticleEmitterWithImageNamed:@"particle"
															  position:Vector2fMake(0, 0)
												sourcePositionVariance:Vector2fMake(50, 30)
																 speed:0.25f
														 speedVariance:0.2f
